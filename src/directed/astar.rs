...
/// Modified astar func with any result and length in result
pub fn astar2<N, C, FN, IN, FH, FS>(
    start: &N,
    mut successors: FN,
    mut heuristic: FH,
    mut success: FS,
    ...
    FS: FnMut(&N, usize) -> bool,
{
    ...
    let path = reverse_path(&parents, |&(p, _)| p, parents.len() - 1);
    return Some((path, Zero::zero()));
}
